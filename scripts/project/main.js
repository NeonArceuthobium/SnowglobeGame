...
// ------------------------------------------------------------------------------------------------
// 4) Z Key Handler: skip or advance
// ------------------------------------------------------------------------------------------------
window.onDialogueKeyPress = function (event) {
    // Only respond to Z key (lowercase or uppercase)
    if (event.key === "z" || event.key === "Z") {
        if (window.isChoiceActive) {
            console.log("🛑 Choices are active — key press ignored.");
            return;
        }

        console.log("⌨️ Z key pressed...");
        if (window.isTyping) {
...
if (!window.isKeyListenerAdded) {
    document.addEventListener("keydown", window.onDialogueKeyPress);
    window.isKeyListenerAdded = true;
    console.log("✅ Dialogue key listener (Z) attached once.");
}
